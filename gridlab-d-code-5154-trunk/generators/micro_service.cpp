...
ref class GlobalObjects2 {
public:
   static MicroturbineSvc::MicroturbineClass^ proxy = gcnew MicroturbineSvc::MicroturbineClass();   
}; 
...
int micro_service::init(OBJECT *parent)
{
	...
	//THE RETURNED VALUE IN THE VARIABLE price
	price= GlobalObjects2 :: proxy ->microturbine_init();
	printf("price = %f  \n", price);
...
TIMESTAMP micro_service::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	int microturbine_presyncResult = GlobalObjects2 :: proxy -> microturbine_presync(0,1);
	printf("microturbine_presyncResult = %d  \n", microturbine_presyncResult);
...
TIMESTAMP micro_service::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	VA_Out.Re() = GlobalObjects2 :: proxy -> microturbine_sync(CircuitA_V_Out_re, CircuitA_V_Out_im, CircuitB_V_Out_re, CircuitB_V_Out_im, CircuitC_V_Out_re, CircuitC_V_Out_im, LineA_V_Out_re, LineA_V_Out_im, LineB_V_Out_re, LineB_V_Out_im, LineC_V_Out_re, LineC_V_Out_im);
	printf("VA_Out.Re() = %f  \n", VA_Out.Re());
return TS_NEVER;
}
