...
ref class GlobalObjects1 {
public:
   static Wind_TurbineSvc::Wind_turbineClass^ proxy = gcnew Wind_TurbineSvc::Wind_turbineClass();   
}; 
...
int wind_service::create(void) 
{
	...
	double windturb_dgcreateResult = GlobalObjects1 :: proxy->windturb_dgcreate();
	printf("windturb_dgcreateResult = %f  \n", windturb_dgcreateResult);
...
int wind_service::init(OBJECT *parent)
{
	...
	//input parameters are the following
	 ...
	 int windturb_dginitResult = GlobalObjects1 :: proxy->windturb_dginit(pLine_I_A_Re, pLine_I_A_Im, pLine_I_B_Re, pLine_I_B_Im, pLine_I_C_Re, pLine_I_C_Im); 
	 printf("windturb_dginitResult = %d  \n", windturb_dginitResult);
...
TIMESTAMP wind_service::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	double windturb_dgsyncResult = GlobalObjects1 :: proxy->windturb_dgsync(pCircuit_V_A_Re, pCircuit_V_A_Im, pCircuit_V_B_Re, pCircuit_V_B_Im, pCircuit_V_C_Re, pCircuit_V_C_Im); 
	printf("windturb_dgsyncResult = %f   \n", windturb_dgsyncResult);
...
TIMESTAMP wind_service::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	//int windturb_dg_postsyncResult = GlobalObjects1 :: proxy->windturb_dg_postsync(0, 0);
