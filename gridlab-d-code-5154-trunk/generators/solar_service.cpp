...
public:
   static SolarSvc::SolarClass^ proxy = gcnew SolarSvc::SolarClass();   
}; 
...
int solar_service::create(void) 
{
	...
	double price = GlobalObjects :: proxy->solarcreate();
	printf("price = %f  \n", price);
...
int solar_service::init(OBJECT *parent)
{
	...
	//int gen_status_v = 0;
	//int panel_type_v = 0;
	climate_result = GlobalObjects :: proxy->solarinit(gen_mode_v, gen_status_v, panel_type_v);
	printf("climate result = %d  \n", climate_result);
...
TIMESTAMP solar_service::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	t0= 0;
	t1 = 1;
	double VA_Out = GlobalObjects :: proxy->solarsync(t0,t1);  
	printf("VA-Out = %f  \n", VA_Out);
//	 *pLine_I =I_Out;
  //   *pCircuit_V =V_Out;
