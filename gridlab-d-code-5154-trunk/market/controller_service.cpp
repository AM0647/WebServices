...
#include "ControllerSvc.h"


 


//ControllerSvc::ControllerClass^ proxy = gcnew ControllerSvc::ControllerClass();



ref class GlobalObjects {
public:
   static ControllerSvc::ControllerClass^ proxy = gcnew ControllerSvc::ControllerClass();   
}; 
...
fetch(&pAvg, avg_target.get_string(), pMarket);
	...
	double COinitResult = GlobalObjects :: proxy->COinit(market_init_price, pstate, market_period, control_mode, psetpoint, base_setpoint, heating_base_setpoint, cooling_base_setpoint);
	printf("Result of init is %f \n", COinitResult);
...
TIMESTAMP controller_service::presync(TIMESTAMP t0, TIMESTAMP t1){
	...
	int COpresyncResult = GlobalObjects :: proxy->COpresync(base_setpoint, psetpoint, control_mode);
	printf("Result of presync is %d \n",COpresyncResult);
...
TIMESTAMP controller_service::sync(TIMESTAMP t0, TIMESTAMP t1){
	...
	double COsyncResult = GlobalObjects :: proxy->COsync(load, demand2, pstate, total, target, pAvg2, pStd2, base_setpoint, control_mode, market_current_frameclearing_price);
	printf("Result of sync is %f \n",COsyncResult);
	return TS_NEVER;
}
...
TIMESTAMP controller_service::postsync(TIMESTAMP t0, TIMESTAMP t1){
	...
	int control_mode=1;
	double base_setpoint=setpoint0;
	double COpostsyncResult = GlobalObjects :: proxy->COpostsync(control_mode,base_setpoint);
	printf("Result of postsync is %f \n",COpostsyncResult);
